"""Is the canvas frozen? Two screenshots 600ms apart after enabling contours + sun nudge."""
import json
...
with sync_playwright() as pw:
    ...
    pg.screenshot(path=OUT + "/frozen_a.png")
    pg.wait_for_timeout(900)
    pg.screenshot(path=OUT + "/frozen_b.png")
    from PIL import Image
    a = Image.open(OUT + "/frozen_a.png").convert('RGB')
    c = Image.open(OUT + "/frozen_b.png").convert('RGB')
    pa, pc = a.load(), c.load()
    diff = sum(1 for y in range(0,800) for x in range(0,1280) if pa[x,y] != pc[x,y])
    print('a-vs-b diff after sun nudge 12.01->12.3:', diff)
    st = pg.evaluate("""() => {
      const out = [];
      window.scene.traverse(o => { if (o.userData && o.userData.isContourOverlay) out.push({type:o.type, verts:o.geometry.attributes.position.count, visible:o.visible}); });
      return { overlays: out, enabled: window._test.contourEnabled,
               frame: window._test.renderer.info.render.frame,
               lines: window._test.renderer.info.render.lines,
               triangles: window._test.renderer.info.render.triangles };
    }""")
    print('scene state:', json.dumps(st))
    px = a.load()
    rib = sum(1 for y in range(52,800) for x in range(280,1280) if abs(px[x,y][0]-77)<14 and abs(px[x,y][1]-54)<14 and abs(px[x,y][2]-23)<16)
    print('ribbon-color px in a:', rib)
    b.close()
