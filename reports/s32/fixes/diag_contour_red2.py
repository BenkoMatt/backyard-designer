"""Decisive: red recolor + depthTest off + NO polygonOffset + lift +3."""
import json
    ...
    pg.evaluate("""() => {
      window.scene.traverse(o => {
        if (o.userData && o.userData.isContourOverlay) {
          ...
          o.material.polygonOffset = false;
          o.material.polygonOffsetFactor = 0;
          const p = o.geometry.attributes.position;
          for (let i = 0; i < p.count; i++) p.setY(i, p.getY(i) + 3);
          p.needsUpdate = true;
        }
      });
    }""")
    pg.wait_for_timeout(700)
    pg.screenshot(path=f"{OUT}/diag_contour_red_lift.png")
    from PIL import Image
    img = Image.open(f"{OUT}/diag_contour_red_lift.png").convert('RGB')
    px = img.load()
    locs=[(x,y) for y in range(52,800) for x in range(280,1280) if px[x,y][0]>180 and px[x,y][1]<90 and px[x,y][2]<90 and not (x>1180 and y<210)]
    print('red px (excl compass):', len(locs), locs[:6])
    if locs:
