...
with sync_playwright() as pw:
    ...
    pg.evaluate("() => window.setMode('advanced')"); pg.wait_for_timeout(600)
    ...
    st = pg.evaluate("""() => {
      ...
      if (!proto) return { err: 'no overlay', toggleExists: !!document.getElementById('ta-contour-toggle'),
        toggleOn: document.getElementById('ta-contour-toggle')?.classList.contains('on'),
        enabled: window._test.contourEnabled,
        terrainMin: window._test.state.terrain ? Math.min(...window._test.state.terrain) : null };
